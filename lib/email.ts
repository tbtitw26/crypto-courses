...
export interface PurchaseEmailData {
  ...
  courseSlug?: string // For course purchases: slug to link to course page
}
          ...
          <div class="email-wrapper">
            ...
            <div class="content">
              ...
              <div class="button-container">
                ...
                ${data.transactionId ? `
                <a href="${(process.env.APP_BASE_URL || config.site.baseUrl)}/api/receipts/${data.transactionId}/pdf" class="button" style="margin-${isArabic ? 'right' : 'left'}: 10px;">${translations.downloadInvoice}</a>
                ` : ''}
                ${data.courseSlug ? `
                <a href="${(process.env.APP_BASE_URL || config.site.baseUrl)}/courses/${data.courseSlug}" class="button" style="margin-${isArabic ? 'right' : 'left'}: 10px;">${translations.viewCourse}</a>
                ` : ''}
              </div>
...
export async function sendPurchaseConfirmationEmail(data: PurchaseEmailData): Promise<void> {
  if (!config.features.emailNotifications) {
    console.log(`[Email] Skipping purchase confirmation email for ${data.userEmail} (notifications disabled)`)
    return
  }
